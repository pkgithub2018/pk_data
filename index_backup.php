  ...
  if(!empty($username) && !empty($password)){
    ...
    */ 
    ...
    $sqlchuser = "SELECT name, psw, email, group_id FROM tbusers 
              WHERE email = '".$_SESSION["username"]."' AND psw = crypt('".$_SESSION["passw"]."', psw)";
    $result = pg_query($con, $sqlchuser) or die(pg_last_error());
    if (pg_num_rows($result) == 0) {
        $message = "Incorrect username or password. Please try again.";
    } else {
        list($namelogin, $pswlogin, $emaillogin, $groupid) = pg_fetch_array($result);
        echo "<script type='text/javascript'>window.location.href = 'main.php?us=$namelogin';</script>";
        exit();
    }
  }
  }
