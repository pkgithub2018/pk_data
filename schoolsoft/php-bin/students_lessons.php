    ...
    <div align="center" class="cominpt" style="width: 100%; float: left; display: inline-block">
		<div style="width: 45%; float: left"> <!-- LEFT SIDE -->
		 ...
		 <h4 align="left" class="lhead">ຊອກ ບົດຮຽນຕາມ</h4>
		  <div class="lhead">
			<form id="frlssid" action="content.php?stud=lesson" method="post">
		  ...
		  <table align="left" class="custb" style='width: 95%; margin-left: 3%'>
			<tr><th>ລດ</th><th>ວິຊາ</th><th>ຊື່ ຫຼື ຫົວຂໍ້ບົດຮຽນ</th><th>ສອນໂດຍ</th><th style='width: 15%'>ອັບໂຫຼດ</th><th>ເບີ່ງ</th></tr>
			  <?php
			  // Subject loop
			  // Search by subject and updated date		  
			    if(!empty($_POST["lbysub"]) || !empty($_POST["lbydate"])){
					...
					if(!empty($_POST["lbysub"])){
					  $srch = $_POST["lbysub"];	
					  //ltype=1 - Lesson
					  $sqllson = "SELECT * FROM tblessons WHERE subid='$srch' AND ltype='1' ORDER BY fileupdate DESC";
					}
					
					if(!empty($_POST["lbydate"])){
					  $srch = $_POST["lbydate"];	
					  $sqllson = "SELECT * FROM tblessons WHERE fileupdate='$srch' AND ltype='1' ORDER BY fileupdate DESC";
					}
					
				} else {
					if(empty($_POST["lbysub"])){
					   $sqllson = "SELECT * FROM tblessons WHERE ltype='1' AND subid IN (SELECT subjid FROM tbteaching WHERE classid='$sclass' GROUP BY subjid) ORDER BY fileupdate DESC";
					}
				}
				...
				while($rl=mysqli_fetch_array($rls)){
					...
					$newdateup =date("d-m-Y", strtotime($dateup));
					 ...
					 if(mysqli_num_rows($rtch)>0){
						  ...
						  print "<tr><td align='center'>$i</td><td>$sublao</td><td>$topicls</td><td>$tnamel</td><td>$newdateup</td><td align='center'><a style='color: black' href='content.php?stud=lesson&sub=$subls&topic=$topicls&dup=$dateup&flpath=$filepath'><i class='fa fa-eye'></a></td></tr>";
					 }
				 } // End of while  
			  } else { // End of if >0
				  $dfound = "nofound";
